...
public class GenericRecordKafkaStreamTest extends KafkaTestBase {
    ...
    void enrichTelemetries() {
        ...
        Serde<InverterTelemetry> inverterTelemetrySerde = getSpecificAvroSerde(false);
        Serde<IrradianceSensorTelemetry> irradianceSensorTelemetrySerde = getSpecificAvroSerde(false);

        final StreamsBuilder builder = new StreamsBuilder();

        ValueJoiner<InverterTelemetry, DeviceController, InverterTelemetry> siteEnricherJoiner = (telemetry, deviceController) -> {
        ...
        builder.stream(INPUT_TOPIC, Consumed.with(stringSerde, inverterTelemetrySerde))
            .selectKey((key, value) -> value.getDeviceControllerSerial().toString())
            .leftJoin(
                deviceControllerKTable,
                siteEnricherJoiner,
                Joined.with(stringSerde, inverterTelemetrySerde, deviceControllerSerde)
            )
            .to(OUTPUT_TOPIC, Produced.with(stringSerde, inverterTelemetrySerde));
        ...
        try (final KafkaStreams streams = new KafkaStreams(builder.build(), getStreamsProperties("telemetry-enricher"))) {
            ...
            newProducer(
                INPUT_TOPIC,
                stringSerde.serializer(),
                inverterTelemetrySerde.serializer(),
                inverterTelemetry -> inverterTelemetry.getDeviceControllerSerial().toString()
            ).accept(List.of(
                new InverterTelemetry(t0, "I10", "DC01", null, d, 123L, 611L, 116L, 721L, 267L),
                new InverterTelemetry(t0, "I18", "DC74", null, d, 193L, 411L, 136L, 729L, 867L),
                new InverterTelemetry(t1, "I72", "DC11", null, d, 822L, 467L, 832L, 128L, 517L),
                new InverterTelemetry(t2, "I79", "DC11", null, d, 422L, 367L, 802L, 122L, 577L),

                new InverterTelemetry(t2, "I10", "DC01", null, d, 123L, 611L, 116L, 721L, 267L),
                new InverterTelemetry(t3, "I18", "DC74", null, d, 197L, 811L, 133L, 429L, 863L),
                new InverterTelemetry(t3, "I72", "DC11", null, d, 852L, 417L, 872L, 138L, 557L),
                new InverterTelemetry(t4, "I79", "DC11", null, d, 522L, 363L, 402L, 127L, 177L)
            ));


            try (KafkaConsumer<String, InverterTelemetry> consumer = newKafkaConsumer("telemetry-enricher-test", stringSerde, inverterTelemetrySerde)) {
                consumer.subscribe(List.of(OUTPUT_TOPIC));

                LinkedList<InverterTelemetry> producedEnrichedTelemetries = new LinkedList<>();

                await().untilAsserted(() -> {
                    ConsumerRecords<String, InverterTelemetry> records = consumer.poll(Duration.ofMillis(200));
                    ...
                    assertThat(producedEnrichedTelemetries)
                        .containsExactlyInAnyOrder(
                            new InverterTelemetry(t0, "I10", "DC01", "X17", d, 123L, 611L, 116L, 721L, 267L),
                            new InverterTelemetry(t0, "I18", "DC74", "X17", d, 193L, 411L, 136L, 729L, 867L),
                            new InverterTelemetry(t1, "I72", "DC11", "A52", d, 822L, 467L, 832L, 128L, 517L),
                            new InverterTelemetry(t2, "I79", "DC11", "A52", d, 422L, 367L, 802L, 122L, 577L),

                            new InverterTelemetry(t2, "I10", "DC01", "X17", d, 123L, 611L, 116L, 721L, 267L),
                            new InverterTelemetry(t3, "I18", "DC74", "X17", d, 197L, 811L, 133L, 429L, 863L),
                            new InverterTelemetry(t3, "I72", "DC11", "A52", d, 852L, 417L, 872L, 138L, 557L),
                            new InverterTelemetry(t4, "I79", "DC11", "A52", d, 522L, 363L, 402L, 127L, 177L)
                        );
                });
            }
        }
